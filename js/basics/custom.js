...
$(document).ready(function() {
    page = {
                ...
                var scene = new ScrollMagic.Scene({triggerElement: '#loader', triggerHook: "onEnter"})
                    ...
                    .on("enter", function (e) {
                        if (!$('#loader').hasClass("active")) {
                            ...
                            setTimeout(addMovies, 1000);
                        }
                    });
                    
                function addMovies() {
                    if ( !$('#movie-list').hasClass('loading-content') ) {
                        $('#movie-list').addClass('loading-content');
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                var obj = JSON.parse(response),
                                    list = obj.movieList,
                                    counter = obj.loadCount;
                                
                                if ( list !== 0 ) {
                                    if ( counter - 20 !== loadCount || counter - 20 === 0 ) {
                                        $('loader').remove();
                                    } else {
                        ...
                        $('#movie-list').removeClass('loading-content');
                    }
                }
        ...
        lazyLoad: function() {
            /*var lazyLoadController = new ScrollMagic.Controller();

            var lazyImages = document.querySelectorAll(".lazy-load");

            for (var i=0; i<lazyImages.length; i++) { // create a scene for each element
                new ScrollMagic.Scene({
                    triggerElement: lazyImages[i]
                })
                .on("enter", function(e) {
                    console.log(lazyImages[i])
                }) // add class toggle
                .addTo(lazyLoadController);
            }

            function lazyLoading(lazyImage) {
